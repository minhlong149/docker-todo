...
- Using Docker can be a bit cumbersome to **manage multiple containers** at the same time. **Docker Compose** is a tool that allows you to do that with a single command.

- The Compose file is a YAML file (`.yml` or `.yaml`) defining services, networks and volumes.
  - **Service** definition contains configuration that is applied to each container started for that service, much like passing command-line parameters to `docker run`.
  - Likewise, **network** and **volume** definitions are analogous to `docker network create` and `docker volume create`.

- To build and run the application, run `docker-compose up`. You can also run the application in the background with the `-d` flag, and close it with `docker compose down`.

### Service configuration reference

> To see the list of all configuration options supported by a service definition in version 3, checkout the [documentation](https://docs.docker.com/compose/compose-file/compose-file-v3/#service-configuration-reference).

- `build` can be specified either as (1) a string containing a path to the directory where the Dockerfile is located, or (2) as an object with the path specified under `context`, and optionally `Dockerfile` and `args`.
  - If you specify `image` as well as build, Compose will built the image and `tag` it with the specified name.
  - If this parameter is not specified, Docker Compose will look for an image with the name of the service in the Docker Hub.

- `port` is the port that the container exposes. It can be specified as a string in the format `<HOST-PORT>:<CONTAINER-PORT>`. *You can also specify the host IP address to bind the port to on the host machine.*

- Your configuration options can so contain `environment` variables.

### Bind mounts

- If we need a file to get it inside the container at startup, we could create a new image FROM an image and COPY it in. But that's not very flexible. Instead, we can use a **bind mount** to mount the file into the container at runtime.

- Bind mount is the act of binding a file (or directory) on the host machine to a file (or directory) in the container.

- The bind mount is declared under key `volumes` in the Compose file, in the format `<HOST-PATH>:<CONTAINER-PATH>`.

> Bind mounts have limited functionality compared to volumes. When you use a **bind mount**, a file or directory on the host machine is mounted into a container. The file or directory is _referenced by its absolute path_ on the host machine. By contrast, when you use a **volume**, a _new directory is created_ within Docker’s storage directory on the host machine, and Docker manages that directory’s contents.

![Types of mounts bind](https://docs.docker.com/storage/images/types-of-mounts-bind.png)

### Persisting data with volumes

- By default, containers are not going to preserve our data. When you close the container, you may or may not be able to get the data back.
...
services:
  mongo:
    volumes:
      - ./*.js:/docker-entrypoint-initdb.d/*.js # bind mount
      - mongo_data:/data/db # volume
